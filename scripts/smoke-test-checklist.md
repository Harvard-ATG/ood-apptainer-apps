...
| 2 | Confirm the secondary image kernel is present under the name the sub-app's `system_default_label` gives it. Both live courses use `Python 3 (System Default — no course packages)`. Confirm it is never the default while the course kernel exists, and that it cannot import course packages. | | |
...
| 3a | For a course that sets **no** `environment_root`, confirm the session starts. The image kernel must be the only kernel, and must become the default. The log must say that no course environment is configured. The log must carry **no** WARNING about a course environment. This is the opt-out path, not the degraded one. Check 3 is the course that asked for an environment and must still warn. | | |
...
| 7a | For a course that sets **no** `environment_root`, confirm code-server starts and writes no `python.defaultInterpreterPath`. Its settings must still carry the image's own keys, and its log must carry no WARNING about a course environment. | | |
