...
fixture_image() {
    ...
    if [ ! -e "$target" ] || [ "$def" -nt "$target" ]; then
        ...
        case "$target" in
            *.sif)
                build_out=$("${OOD_APPTAINER_BIN:-apptainer}" build --fakeroot "$tmpbuild.sif" "$def" 2>&1) || {
                ...
                ;;
            *)
                build_out=$("${OOD_APPTAINER_BIN:-apptainer}" build --fakeroot --sandbox "$tmpbuild.dir" "$def" 2>&1) || {
                    printf 'fixture_image: apptainer build failed: %s\n' "$build_out" >&2
                    rm -rf "$tmpbuild.dir"
                    return 1
                }
